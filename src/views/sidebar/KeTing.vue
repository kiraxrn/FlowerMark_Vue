<template>
  <div class="classify-content">
    ...
    <div class="breadcrumbs">
      ...
      <router-link to="/classify">商品分类</router-link> &gt; 客厅装饰植物
    </div>
  ...
  data() {
    ...
    methods: {
      ...
      async fetchProducts() {
        this.loading = true;
        try {
          console.log('=== KeTing.vue 客厅商品数据请求开始 ===');
          console.log('KeTing.vue 调用 commodityAPI.getCommoditiesByCategory(3)');
          
          const response = await commodityAPI.getCommoditiesByCategory(3);
          
          console.log('KeTing.vue API响应数据:', response);
          console.log('KeTing.vue 响应状态码:', response.code);
          console.log('KeTing.vue 响应消息:', response.message);
          console.log('KeTing.vue 响应数据结构:', response.data);
          ...
          if (response.code === 200) {
            ...
            console.log('KeTing.vue 客厅商品数据加载成功，商品数量:', this.products.length);
            console.log('KeTing.vue 商品列表详情:', this.products);
            console.log('KeTing.vue 商品数据结构:', {
            ...
            console.warn('KeTing.vue 商品数据加载失败，状态码:', response.code);
            this.$message.error(response.message || '商品数据加载失败');
          }
        } catch (error) {
          console.error('KeTing.vue 获取客厅商品数据失败:', error);
          console.error('KeTing.vue 错误详情:', {
            消息: error.message,
            响应: error.response,
          ...
          if (process.env.NODE_ENV === 'development') {
            console.log('KeTing.vue 使用模拟数据');
            // 使用模拟数据
            this.products = this.useFallbackData();
            this.filteredProducts = [...this.products]; // 初始化 filteredProducts
            console.log('KeTing.vue 模拟商品数据:', this.products);
            this.$message.info('已使用模拟数据');
            this.applySorting();
            this.totalProducts = this.filteredProducts.length;
          }
        } finally {
          console.log('=== KeTing.vue 客厅商品数据请求结束 ===');
          this.loading = false;
        }
      },
    ...
    useFallbackData(){
      return[
        { id: 301, name: '发财树盆栽', price: 78.0, image: 'https://img1.baidu.com/it/u=234567890,234567890&fm=253&fmt=auto&app=120&f=JPEG?w=500&h=664' },
        { id: 302, name: '绿萝吊篮', price: 35.0, image: 'https://img2.baidu.com/it/u=234567890,234567890&fm=253&fmt=auto&app=120&f=JPEG?w=500&h=664' },
        { id: 303, name: '龟背竹', price: 65.0, image: 'https://img3.baidu.com/it/u=234567890,234567890&fm=253&fmt=auto&app=120&f=JPEG?w=500&h=664' },
        { id: 304, name: '虎皮兰', price: 42.0, image: 'https://img4.baidu.com/it/u=234567890,234567890&fm=253&fmt=auto&app=120&f=JPEG?w=500&h=664' },
        { id: 305, name: '白掌盆栽', price: 58.0, image: 'https://img5.baidu.com/it/u=234567890,234567890&fm=253&fmt=auto&app=120&f=JPEG?w=500&h=664' },
        { id: 306, name: '金钱树', price: 95.0, image: 'https://img6.baidu.com/it/u=234567890,234567890&fm=253&fmt=auto&app=120&f=JPEG?w=500&h=664' },
      ]
    }
  }
}
